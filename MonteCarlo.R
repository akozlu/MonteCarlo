...
for (i in 1:num.its) { #Start the simulation
  ...
  longest_wait_time <- 0 
  longest_waiting_passenger <- vector()
  for(row in 1:nrow(matrix_positions)) { # for each position
    position<- matrix_positions[row,]
    r <- position[1] # get row of matrix position in fm$solution
    col <- position[2] # get column of matrix position in fm$solution
    ...
    if(distance >longest_wait_time ) { # If distance was larger than our longest waiting time 
      longest_wait_time <- distance # we update the longest waiting time 
      longest_waiting_passenger <- r # we update passenger with longest waiting time 
    }
  }
  # Now we want to check if any passenger waits for a car and gets dropped off before last Passenger is picked up 
  ...
  total_times <- total_times[-longest_waiting_passenger] # We have to exclude the time for the longest waiting passenger
  waiting_time_of_longest_passenger <- p.list[[longest_waiting_passenger]]$WaitTime # get waiting time of the 12th passenger
  greater_than_1 <- sum(total_times <= waiting_time_of_longest_passenger)
  if(greater_than_1 == 0 ) {
